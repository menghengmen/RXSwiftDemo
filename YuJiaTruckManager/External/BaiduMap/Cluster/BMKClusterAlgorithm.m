...
#define MAX_DISTANCE_IN_DP    50 //300dp
 ...
 * @return BMKCluster数组
 */
- (NSArray*)getClusters:(CGFloat) zoomLevel visibleMapRect:(BMKMapRect)visibleMapRect {
    ...
    @synchronized(_quadtree) {
        for (BMKQuadItem *candidate in _quadItems) {
            ...
            if (BMKMapRectContainsPoint(visibleMapRect, BMKMapPointForCoordinate(candidate.clusterItem.coor)) == NO) {
                continue;
            }
